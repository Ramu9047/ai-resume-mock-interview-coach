...
export default function SummaryPage() {
  ...
  // Calculate per-category averages and percentage bar heights (0-100%)
  ...
  const interviewBars = Object.entries(byCategory).map(([cat, scores]) => {
    const avg = scores.reduce((a, b) => a + b, 0) / scores.length
    return {
      key: cat === 'technical' ? 'TEC' : cat === 'behavioral' ? 'BEH' : 'ROL',
      score: Math.round(avg * 10), // Convert 0-10 category average to 0-100 percentage for bar height
    }
  })
      ...
      <div className="page-wrapper w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <button
            id="print-report-btn"
            onClick={handlePrint}
            className="btn-secondary text-xs gap-1.5 py-2 px-4 shrink-0 self-start sm:self-auto shadow-xs print:hidden"
          >
        ...
        {/* Overall score hero with Signal Bars score visualization */}
        <div className="p-8 mb-6 flex flex-col sm:flex-row items-center gap-8
                        bg-[#131316] border border-[#26262B] shadow-[0_4px_12px_rgba(0,0,0,0.4)] rounded-lg border-t-2 border-t-[#FF5A1F] animate-slide-up print:p-4 print:mb-4 print:gap-4 print:bg-white print:border-[#E5E7EB]">
          {/* Signal Bars Visualization */}
          <div className="w-full sm:w-72">
            <CircularProgress
              score={overallScore}
              scaleMax={10}
              customBars={interviewBars.length > 0 ? interviewBars : null}
                ...
                {Object.entries(byCategory).map(([cat, scores]) => {
                  ...
                  return (
                    <div key={cat} className={`${badgeClass} gap-1.5 py-1 px-2.5 shadow-xs`}>
                      ...
                      <span className="font-bold font-mono">{avg.toFixed(1)}/10</span>
                    </div>
                  )
                })}
              </div>
        ...
        {/* CTA row */}
        <div className="flex flex-col sm:flex-row items-center gap-4 pt-4 border-t border-[#26262B] print:hidden">
          <button
            id="try-another-btn"
            onClick={() => navigate('/')}
